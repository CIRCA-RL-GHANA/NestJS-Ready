    ...
    await queryRunner.createTable(
      new Table({
        ...
        columns: [
          ...
          { name: 'plan_type', type: 'enum', enum: ['FREE', 'BASIC', 'PREMIUM', 'ENTERPRISE'], default: "'FREE'" },
          { name: 'plan_price', type: 'decimal', precision: 10, scale: 2 },
          { name: 'billing_cycle', type: 'enum', enum: ['MONTHLY', 'QUARTERLY', 'ANNUAL'], default: "'MONTHLY'" },
          { name: 'start_date', type: 'timestamp' },
          { name: 'end_date', type: 'timestamp' },
          { name: 'renewal_date', type: 'timestamp', isNullable: true },
          { name: 'auto_renew', type: 'boolean', default: true },
          { name: 'status', type: 'enum', enum: ['ACTIVE', 'PAUSED', 'CANCELLED', 'EXPIRED'], default: "'ACTIVE'" },
